...
for baseNum=[10 50 100 363] 
    imgIndex=1;
    for i=[1 8 15] % img number in train set
        reconTrain(1:baseNum,baseNumIndex,imgIndex) = (trainSetDiff(:,i).' * mEigVec(:,363-baseNum+1:363)).';
        imgIndex = imgIndex+1;
    end

    for i=1:3
        %for j=1:baseNum %change 364 here
        reconTrainImg(:,baseNumIndex,i) = (reconTrain(1:baseNum,baseNumIndex,i).' * mEigVec(:,363-baseNum+1:363).').';
        %end
